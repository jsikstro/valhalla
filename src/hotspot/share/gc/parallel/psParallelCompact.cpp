    ...
    static void forward_objs_in_range(ParCompactionManager* cm,
      ...
      while (cur_addr < end) {
        ...
        if (new_addr != cur_addr) {
          const size_t remaining_words = (size_t)(end - cur_addr);
...
void MoveAndUpdateClosure::reinit_mark(oop source, oop copy_destination, bool safe_header) {
  ...
  if (UseCompactObjectHeaders || (EnableValhalla && safe_header)) {
    copy_destination->set_mark(source->klass()->prototype_header());
  } else {
...
void MoveAndUpdateClosure::do_addr(HeapWord* addr, size_t words, bool safe_header) {
  ...
  if (copy_destination() != source()) {
    ...
    Copy::aligned_conjoint_words(source(), copy_destination(), words);
    reinit_mark(cast_to_oop(source()), cast_to_oop(copy_destination()), safe_header);
  }
